...
6. Go back to https://usa.ipums.org/usa and click `Get Data`. You will be taken to the IPUMS data selection interface. Here you can select the variables (columns) you want to include as well as the samples (surveys) that you want to include.  You can also click on variables to get more information about them.

7. In the `HOUSEHOLD` dropdown menu, click `TECHNICAL`. Click on the variable named `SERIAL` and read the description. `SERIAL` is the identification number for a household in the survey.
...
Now you know how to navigate the variable selection screen. You are now ready to create a data extract. The first step is to select the variables you want to include.

11. First, let's take a look at which variables are preselected by default. 
    ...
    The preselected variables contain technical data about survey sampling which are necessary for calculating accurate statistics. For most of this class, we'll only need to worry about: `YEAR`, `SERIAL`, `PERNUM`, and `PERWT`. 
...
14. Go to `PERSON`->`EDUCATION` and add `EDUC` and `DEGFIELD` to your cart. Click on the variables to find out more about each variable. Don't forget to look at the detailed codes for `EDUC`.
...
24. The file type is `.gz`, which is a type of compression like `.zip`. Extract it using any Windows program compatible with `.gz` files, like 7-zip or WinRAR. If you are using a Mac, you can use an app called TheUnarchiver. You can also use an online utility like https://www.ezyzip.com/unzip-tar-gz-file-online.html. The resulting .csv file should be about 44 MB in size.
...
26. Los Angeles county is represented in the data by `COUNTYFIP` code 37. Try using Excel to calculate the average wage and salary income (`INCWAGE`) for employed people between the ages of 25 and 65 in Los Angeles county in 2019. Remember that because the ACS is a stratified sample, we need to calculate the average using the sampling weights. 
    ...
    *Note: This exercise is extra credit, so don't worry if you can't figure it out.*
